...
bool http_packet::decode(){
	...
	return decode_http_header(_http_header.to_string());
}
...
#define FIND_METHOD(x) (start=http_header.find(#x))!=std::string::npos
...
const std::vector<char>& http_packet::encode(const std::string& body){
	...
	set_header_body();
	std::cout<<"Pakcet:["<<(char*)&_http_packet_data[0]<<"]"<<std::endl;
